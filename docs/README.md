...
CM2D provides image format support through both native and optional ImageMagick integration.

### Native Format Support
- **TGA (Targa)** - Fast, uncompressed format (built-in)
- Platform-specific formats (Windows DIB, macOS CoreGraphics, X11, Wayland)

### ImageMagick Integration (Optional)

Enable with `-DCM2D_USE_IMAGEMAGICK=ON` for 200+ formats (PNG/JPEG/BMP/GIF/TIFF/WebP/...).
API: `inc/hgl/2d/BitmapImageMagick.h`（`LoadBitmapRGBA8FromImageMagick` / `SaveBitmapToImageMagick` 等）。
